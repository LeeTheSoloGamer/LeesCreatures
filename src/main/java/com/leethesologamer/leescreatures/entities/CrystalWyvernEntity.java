...
public class CrystalWyvernEntity extends TameableEntity implements IAnimatable, IFlyingAnimal {
	...
	private static final DataParameter<BlockPos> HOME_POS = EntityDataManager.createKey(CrystalWyvernEntity.class, DataSerializers.BLOCK_POS);
	private static final DataParameter<Boolean> GOING_HOME = EntityDataManager.createKey(CrystalWyvernEntity.class, DataSerializers.BOOLEAN);
	private static final DataParameter<BlockPos> TRAVEL_POS = EntityDataManager.createKey(CrystalWyvernEntity.class, DataSerializers.BLOCK_POS);
    private static final DataParameter<Byte> CONTROL_STATE = EntityDataManager.createKey(CrystalWyvernEntity.class, DataSerializers.BYTE);
	private static final DataParameter<Boolean> TRAVELLING = EntityDataManager.createKey(CrystalWyvernEntity.class, DataSerializers.BOOLEAN);
	...
	protected void registerGoals() {
		...
		this.goalSelector.addGoal(1, new ChompAttackGoal());
	    this.goalSelector.addGoal(2, new TamedAiRide(this, 1.1D));
		this.goalSelector.addGoal(3, new WyvernFindHomeGoal(this, 1.1D));
		this.goalSelector.addGoal(4, new WyvernTravelGoal(this, 1.1));
	...
	public static boolean canCrystalWyvernSpawn(EntityType<CrystalWyvernEntity> entity, IWorld worldIn, SpawnReason reason, BlockPos pos, Random randomIn ){
		...
		return (blockstate.isIn((ITag<Block>) Blocks.STONE) || blockstate.isIn((ITag<Block>) Blocks.SNOW) || blockstate.isIn((ITag<Block>) Blocks.AIR)) && worldIn.getLightSubtracted(pos, 0) > 8;
	}
	...
	@Nullable
	protected ResourceLocation getLootTable(CrystalWyvernEntity entity) {
    ...
    public PlayerEntity getRidingPlayer() {
	    ...
	    protected void updateClientControls() {
	        ...
	        if (this.isRidingPlayer(mc.player)) {
	            ...
	            goDown(mc.gameSettings.keyBindBack.isKeyDown());
	...
	static class WyvernFindHomeGoal extends Goal {
		...
		public void tick() {
			...
			if (this.wyvern.getNavigator().noPath()) {
				...
				if (vector3d1 != null && !flag && !this.wyvern.world.getBlockState(new BlockPos(vector3d1)).isIn((ITag<Block>) Blocks.AIR)) {
